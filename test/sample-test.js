...
describe("TestModifyVariable", function () {
  it("should change x to 1337", async function () {
    ...
    const contract = await ModifyVariable.deploy(10, "Marc");
  ...
  it("should change name to passed", async function () {
    ...
    // modify name from Marc to passed with this function
    await contract.modifyName();

    // getter for the name state variable
    const newName = await contract.name();
    assert.equal(newName.toString(), "passed");
  });
});
